...
app.get('/details', async function (req, res) {
    ...
    try {
        ...
        results = await(pool.query("SELECT u.id, u.firstname, u.middlename, u.lastname, u.email, u.phone, u.address, c.name as custname, r.name as role FROM customers as c JOIN users as u ON u.cid=c.cid JOIN role as r ON r.key=u.rid ORDER BY u.id"));
...
app.post('/details', async function (req, res) {
    try {
        await(pool.query("INSERT INTO users (id, firstname, middlename, lastname, email, phone, address, cid, rid)" + 
                            "VALUES ('" + req.body.id + "','" + req.body.firstname + "','" + req.body.middlename + "','" + 
                                            req.body.lastname + "','" + req.body.email + "','" + req.body.phone + "','" + 
                                                req.body.address + "','" + req.body.cid + "','" + req.body.rid + "')"));
    }
    catch (e) {
        console.log(e);
    }
